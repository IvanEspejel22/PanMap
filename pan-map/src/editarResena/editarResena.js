...
const renderField = ({ input, label, type, placeholder, meta: { touched, error, warning } }) => (
  <div>
    ...
    <div>
      <input className="input-titulo-nr2" {...input} placeholder={placeholder} type={type}/><br/>
      {touched && ((error && <span className="alertas-nr">{error}</span>) || (warning && <span className="alertas-nr">{warning}</span>))}
    </div>
  </div>
)

const renderField2 = ({ input, label, type, placeholder, value,meta: { touched, error, warning } }) => (
  <div>
    <small className="titulo-nueva-resena">{label}</small>
    <div>
      <input className="input-titulo-nr" {...input} placeholder={placeholder} type={type}/><br/>
      {touched && ((error && <span className="alertas-nr">{error}</span>) || (warning && <span className="alertas-nr">{warning}</span>))}
    </div>
  </div>
)
...
const editarResena= ({ fotos, titulo2, resena,temas,calificacion, fecha,autor , panaderia , direccion,id,onSubmit },props) => {
    ...
    <section className="nr-section">
      ...
      <div id="Hero-detalle">
        ...
        <div className="detalle-caja">
          ...
          <Field name="titulo" type="text" component={renderField2} label="Titulo" placeholder={titulo2}/>
          </div>
          <div>
              <input type="file" />
              {/* <button>
      ...
      <div className="contenido-detalle">
        <div className="info-detalle-nr">
          <div className="detalle-subtitulo">
            <div>
              <Field name="panaderia" type="text" component={renderField} placeholder={panaderia} label="Panaderia"/>
            </div>
            <div>
              <Field name="autor" type="number" component={renderField} placeholder={autor} label="Autor"/>
            </div>
            <div>
          ...
          <div className="detalle-resena small-nueva-resena">
            ...
            <Field name="resena" className="resena-nr" component="textarea" placeholder={resena}/>
          </div>
         <div className="categorias-nr">
         <FieldArray name="temas" component={renderHobbies}/>
          </div>
          <div className="">
          <button type="submit" className="btn-nr" disabled={submitting}>Editar Reseña</button>
          ...
          <div className="detalle-direccion-nr">
            ...
            <Field name="calle" type="text" component={renderField}  placeholder={direccion.calle} label="Calle"/>
            <Field name="numero" type="number" component={renderField}  placeholder={direccion.numero} label="Numero"/>
            <Field name="alcaldia" type="text" component={renderField}  placeholder={direccion.alcaldia} label="Alcaldia"/>
            <Field name="cp" type="number" component={renderField}  placeholder={direccion.cp} label="Codigo postal"/>
            <Field name="estado" type="text" component={renderField}   placeholder={direccion.estado}label="Estado"/>
          </div>
...
export default reduxForm({
  form: 'editararResena',  // a unique identifier for this form
